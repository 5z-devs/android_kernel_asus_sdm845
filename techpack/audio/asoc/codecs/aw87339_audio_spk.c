...
static unsigned char aw87339_audio_drcv_spk(void);
static unsigned char aw87339_audio_abrcv_spk(void);
static unsigned char aw87339_audio_rcvspk_spk(void);
...
static int aw87339_spk_status = 0;
...
unsigned char aw87339_audio_kspk_spk(void)
{
	if(!aw87339_spk_status){
		printk("%s[SPK] spk not init", __func__);
		return -1;
	}
...
EXPORT_SYMBOL(aw87339_audio_kspk_spk);
...
EXPORT_SYMBOL(aw87339_audio_drcv_spk);
...
EXPORT_SYMBOL(aw87339_audio_off_spk);
...
static int aw87339_parse_dt(struct device *dev, struct device_node *np) {
    ...
    if (aw87339->reset_gpio < 0) {
        ...
        dev_info(dev, "%s: reset gpio provided ok %d\n", __func__,aw87339->reset_gpio);
    }
    return 0;
}
