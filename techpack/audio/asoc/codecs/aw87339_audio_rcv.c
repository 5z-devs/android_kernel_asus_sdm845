 ...
 * aw87339 marco
 ******************************************************************************/
#define AW87339_I2C_NAME    "aw87339_pa_rcv"
 ...
 * aw87339 functions
 ******************************************************************************/
unsigned char aw87339_audio_off_rcv(void);
static unsigned char aw87339_audio_kspk_rcv(void);
unsigned char aw87339_audio_drcv_rcv(void);
static unsigned char aw87339_audio_abrcv_rcv(void);
unsigned char aw87339_audio_rcvspk_rcv(void);
...
static struct aw87339_t *aw87339;
static int aw87339_rcv_status = 0;
...
0x39, 0x0E, 0xA0, 0x06, 0x05, 0x10, 0x07, 0x52, 0x06, 0x08, 0x96
};
static unsigned char aw87339_drcv_cfg_default[] = {
0x39, 0x0E, 0xA8, 0x06, 0x05, 0x00, 0x0f, 0x52, 0x09, 0x08, 0x97
};
static unsigned char aw87339_abrcv_cfg_default[] = {
0x39, 0x0E, 0xAC, 0x06, 0x05, 0x00, 0x0f, 0x52, 0x09, 0x08, 0x97
};
static unsigned char aw87339_rcvspk_cfg_default[] = {
0x39, 0x0E, 0xB0, 0x06, 0x05, 0x00, 0x07, 0x52, 0x06, 0x08, 0x96
};
...
static unsigned char i2c_write_reg(unsigned char addr, unsigned char reg_data)
{
    ...
    if(NULL == aw87339->i2c_client) {
        pr_err("%s: [RCV] aw87339 i2c_client is NULL\n", __func__);
        return -1;
    }

    ret = i2c_transfer(aw87339->i2c_client->adapter, msgs, 1);
    if (ret < 0)
        pr_err("%s: [RCV] i2c read error: %d\n", __func__, ret);
...
static unsigned char aw87339_hw_on(void)
{
    ...
    if (aw87339 && gpio_is_valid(aw87339->reset_gpio)) {
        ...
        dev_err(&aw87339->i2c_client->dev, "%s: [RCV] failed\n", __func__);
    }
...
static unsigned char aw87339_audio_kspk_rcv(void)
{
	if(!aw87339_rcv_status){
		printk("%s[RCV] rcv not init", __func__);
		return -1;
	}
...
EXPORT_SYMBOL(aw87339_audio_kspk_rcv);
...
EXPORT_SYMBOL(aw87339_audio_drcv_rcv);
...
EXPORT_SYMBOL(aw87339_audio_rcvspk_rcv);
...
EXPORT_SYMBOL(aw87339_audio_off_rcv);
...
static void aw87339_rcvspk_cfg_loaded(const struct firmware *cont, void *context)
{
    ...
    if (!cont) {
        pr_err("%s:[RCV] failed to read %s\n", __func__, aw87339_rcvspk_name);
        release_firmware(cont);
        return;
    }

    pr_info("%s:[RCV] loaded %s - size: %zu\n", __func__, aw87339_rcvspk_name,
                    cont ? cont->size : 0);

    for(i=0; i<cont->size; i++) {
        pr_info("%s:[RCV] cont: addr:0x%02x, data:0x%02x\n", __func__, i, *(cont->data+i));
    }

    aw87339_rcvspk_cnt = kzalloc(cont->size+sizeof(int), GFP_KERNEL);
    if (!aw87339_rcvspk_cnt) {
        release_firmware(cont);
        pr_err("%s:[RCV] error allocating memory\n", __func__);
        return;
    }
    ...
    for(i=0; i<aw87339_rcvspk_cnt->len; i++) {
        pr_info("%s:[RCV] rcv_cnt: addr:0x%02x, data:0x%02x\n",
...
static void aw87339_abrcv_cfg_loaded(const struct firmware *cont, void *context)
{
    ...
    if (!cont) {
        ...
        if(ret) {
            ...
            pr_err("%s:[RCV] request_firmware_nowait failed with read %s",
                    __func__, aw87339_rcvspk_name);
        }
        return;
    }
...
static void aw87339_kspk_cfg_loaded(const struct firmware *cont, void *context)
{
    ...
    for(i=0; i<aw87339_kspk_cnt->len; i++) {
        pr_info("%s:[RCV] spk_cnt: addr:0x%02x, data:0x%02x\n",
...
static ssize_t aw87339_get_reg(struct device* cd,struct device_attribute *attr, char* buf)
{
    ...
    for(i=0; i<AW87339_REG_MAX ;i++) {
        if(aw87339_reg_access[i] & REG_RD_ACCESS) {
            ...
            len += snprintf(buf+len, PAGE_SIZE-len, "[RCV] reg:0x%02x=0x%02x\n", i,reg_val);
        }
    }
...
static ssize_t aw87339_set_mode(struct device* cd, struct device_attribute *attr, const char* buf, size_t len)
{
    ...
    if(databuf[0] == 0) {
        ...
        aw87339_audio_kspk_rcv();
    } else if(databuf[0] == 2) {
        aw87339_audio_drcv_rcv();
    } else if(databuf[0] == 3) {
        aw87339_audio_abrcv_rcv();
    } else if(databuf[0] == 4) {
...
static int aw87339_parse_dt(struct device *dev, struct device_node *np) {
    ...
    if (aw87339->reset_gpio < 0) {
        dev_err(dev, "%s:[RCV]  no reset gpio provided\n", __func__);
        return -1;
    } else {
        dev_info(dev, "%s:[RCV] reset gpio provided ok %d\n", __func__,aw87339->reset_gpio);
    }
    return 0;
}
...
static int aw87339_read_chipid(void)
{
    ...
    while(cnt < AW_READ_CHIPID_RETRIES) {
        ...
        if(reg == 0x39) {
            pr_info("%s:[RCV] aw87339 chipid=0x%x\n", __func__, reg);
            return 0;
        } else {
            pr_info("%s:[RCV] aw87339 chipid=0x%x error\n", __func__, reg);
        }
        cnt ++;
...
static int aw87339_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
        dev_err(&client->dev, "%s:[RCV] check_functionality failed\n", __func__);
        err = -ENODEV;
    ...
    if (np) {
        ...
        if (ret) {
            dev_err(&client->dev, "%s:[RCV] failed to parse device tree node\n", __func__);
    ...
    if (gpio_is_valid(aw87339->reset_gpio)) {
        ret = devm_gpio_request_one(&client->dev, aw87339->reset_gpio,
            GPIOF_OUT_INIT_LOW, "[RCV] aw87339_rst");
        if (ret){
            dev_err(&client->dev, "%s:[RCV] rst request failed\n", __func__);
    ...
    if (ret < 0) {
        dev_err(&client->dev, "%s:[RCV] aw87339_read_chipid failed ret=%d\n", __func__, ret);
        err = -EIO;
    ...
    if (ret < 0) {
        dev_info(&client->dev, "%s[RCV] error creating sysfs attr files\n", __func__);
    }
...
static int __init aw87339_pa_init(void) {
    ...
    if (ret) {
        printk("****[%s][RCV]  Unable to register driver (%d)\n",
                __func__, ret);
        return ret;
    }
    return 0;
